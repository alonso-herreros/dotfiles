...
# Alonso Herreros' custom version
#
# # README
#
...
prompt_context() {
  local seg="%(!.%{%F{$AGNOSTER_STATUS_ROOT_FG}%}.)"

  if [[ "$(hostname)" != "$DEFAULT_HOSTNAME" || -n "$SSH_CLIENT" ]]; then
    seg="$seg%n@%m"
    ...
    # return # Avoid calling prompt_segment at all
    seg="$seg%n@%m"

  fi

  prompt_segment "$AGNOSTER_CONTEXT_BG" "$AGNOSTER_CONTEXT_FG" "$seg"
}
